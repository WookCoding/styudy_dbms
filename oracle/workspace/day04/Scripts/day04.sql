...
SELECT SUM(NVL(HEIGHT,170)) / COUNT(PLAYER_ID) FROM PLAYER;

/*[플래티넘]*/
/*DEPT 테이블의 LOC별 평균 급여를 반올림한 값과 각 LOC별 SAL 총 합을 조회, 반올림 : ROUND()*/
SELECT LOC, ROUND(AVG(SAL),2), SUM(SAL) FROM DEPT D
JOIN EMP E
...
SELECT P1.TEAM_ID , P1.PLAYER_NAME, P1.WEIGHT  
FROM  
(
   SELECT TEAM_ID, MAX(WEIGHT) WEIGHT FROM PLAYER
   GROUP BY TEAM_ID
) P2 JOIN PLAYER P1
...
SELECT NVL(LAST_NAME, 'NO') || ' ' || NVL(FIRST_NAME,'NAME') NAME , D.*
 ...
 *  실제 데이터가 저장되는 것은 아니지만 VIEW를 통핵서 데이터를 관리할 수 있다.
 * 
...
SELECT E1.LAST_NAME || ' ' || E1.FIRST_NAME EMPLOYEE_NAME, E2.LAST_NAME || ' ' || E2.FIRST_NAME MANAGER_NAME
