...
SELECT PLAYER_NAME, BACK_NO AS "등 번호", NICKNAME "선수 별명" FROM PLAYER;

/*위 결과에서 NICKNAME이 NULL일 경우 "없음"으로 변경*/
SELECT PLAYER_NAME, BACK_NO "등 번호", NVL(NICKNAME,'없음') "선수 별명" FROM PLAYER;
...
SELECT PLAYER_NAME || '의 영어이름은 ' || E_PLAYER_NAME || '이다' 영어이름소개 FROM PLAYER;

/*PLAYER_NAME의 포지션은 "POSITION"입니다.*/
SELECT PLAYER_NAME || '의 포지션은 ' || NVL("POSITION", '미정') || '입니다.' AS 포지션소개 FROM PLAYER;
...
SELECT AVG(HEIGHT), MAX(HEIGHT), SUM(HEIGHT), COUNT(HEIGHT)  FROM PLAYER;

/*PLAYER 테이블에서 HEIGHT로 총 선수 명수 검색*/
SELECT COUNT(NVL(HEIGHT, 0)) AS "총 선수 명수" FROM PLAYER;
...
/*EMPLOYEES 테이블에서 JOB_ID별 평균 SALARY가 10000미만인 JOB_ID 검색, JOB_ID 알파벳 순정렬*/
...
/*PLAYER 테이블에서 전체 평균 키와 포지션별 평균 키 구하기*/
	SELECT "POSITION", AVG(HEIGHT) AS "포지션별 평균 키", (SELECT AVG(HEIGHT) FROM PLAYER) "전체 평균 키" FROM PLAYER
	WHERE HEIGHT IS NOT NULL
	GROUP BY "POSITION";
